...
def main():
    ...
    parser.add_argument("-t", "--monomer_template", help="monomer template DNA sequence in fasta format "
                                                         "for stringdecomposer to build block, required",required=True)
    parser.add_argument("-tm", "--template_monomers", help="template monomers DNA sequence in "
                                                           "fasta format for transforming block to monomer", required=True)

    parser.add_argument("-o", "--output_dir", help="HiCAT output path default is ./HiCAT_out",default='./HiCAT_out',required=False)

    parser.add_argument("-ms", "--min_similarity", help="The lower bound for similarity threshold between block and target monomer, default is 0.9", type=float, default=0.9,required=False)
    ...
    if overlap_threshold > 500:
        ...
        cmd = 'python ' + \
              ...
              ' -v ' + str(overlap_threshold) + ' -o ' + output_dir + ' -t ' + str(thread)
    else:
        cmd = 'python ' + \
              script_path + '/stringdecomposer/bin/stringdecomposer' + ' ' + \
              input_fasta + ' ' + monomer_template + \
              ' -o ' + output_dir + ' -t ' + str(thread)
